...
2. 构造 $M_{\text{forged}} = \text{pad}(\text{"hello\_world"}) \parallel (T_1 \oplus \text{pad}(\text{"get\_flag"}))$
...
AES 是分组密码，每块固定 16 字节，不够就必须补齐。Writeup 中的 $\text{pad}(M_1)$ 就是指补零后的完整块。

---

## Q4：为什么 $\text{MAC}(M_1 \| M_2) = C_2$？
...
块2: [10 89 7b 80 7f 77 71 76 b9 74 86 4b ...]   ← T1 ⊕ pad("get_flag")
```
...
## Q7：$\text{MAC}(M_1)$ 就是对 $M_1$ 进行 AES 加密吗？
