...
- 这样一来，RSA 赖以成立的“分解 `N = p * q` 很困难”这个前提就直接失效了。
...
??? note "知识点常见问题解答（可选）"
    ...
    ```text
    R_k * 10^offset + multiplier * R_k
    = R_k * (10^offset + multiplier)
    ```

    本题里 `multiplier = 7`，所以直接得到：
...
- **详细步骤**：
  ...
  3. 构造 `p=(10^317-1)/9` 和 `q=10^412+7`
  4. 计算 `d` 并解密 `c`
...
- **详细步骤**：
  ...
  3. 使用 `math/big` 构造 `p=(10^k-1)/9` 和 `q=10^(t+k)+7`
  4. 计算 `d` 并解密出明文
