...
```text
input_seed ^ 0x1234567890abcdef1234567890abcdef
```
...
```text
阅读 server.py -> 发现用户可控 seed 会先与固定常量异或 -> 注意到题目未禁止 xoroshiro128+ 全零状态 -> 输入与掩码相同的十进制数使内部状态归零 -> 99 个 NPC 全部出 Rock -> 玩家出 Paper 成为唯一赢家 -> 获得 Flag
```
